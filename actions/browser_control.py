...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".lite_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".lite_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using LITE profile")
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".lite_profiles" / "safari")
        ...
        # LITE otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        lite_profile = str(Path.home() / ".lite_profiles" / self.browser_name)
        Path(lite_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with LITE profile: {lite_profile}")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "lite_screenshot.png")
