...
Supported types:
  ...
  video   → trim, extract_audio, extract_frame, info, compress, transcribe, summarize
  zip     → list, extract
...
def _process_video(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "summarize":
        ...
        if speak:
            speak("Uploading the video and processing it — this can take a minute or two for longer files.")

        try:
            myfile = client.files.upload(file=str(path))
        except Exception as e:
            return f"Video upload to Gemini failed: {e}"

        # Video files need server-side processing before they're usable —
        # unlike the inline-bytes approach used for short audio clips above,
        # which has no such wait but also caps out around 20MB.
        wait_start = _time.time()
        try:
            while myfile.state and myfile.state.name == "PROCESSING":
                if _time.time() - wait_start > 300:  # 5 min safety cap
                    return "Video processing timed out on Gemini's side — try a shorter clip."
                _time.sleep(4)
                myfile = client.files.get(name=myfile.name)
        except Exception as e:
            return f"Video processing check failed: {e}"

        if myfile.state and myfile.state.name == "FAILED":
            return "Gemini couldn't process this video file (unsupported format or corrupt file)."

        prompt = (
            "Watch and listen to this video. Respond in exactly this format:\n\n"
            "TRANSCRIPT:\n<a full, accurate transcript of all spoken content, noting "
            "speaker changes if audible>\n\n"
            "SUMMARY:\n<a concise summary of what the video covers — key points, any "
            "on-screen text or visuals worth noting, and the overall takeaway>"
        )
        try:
            ...
            return f"Video summarization failed: {e}"
        finally:
            try:
                client.files.delete(name=myfile.name)
            except Exception:
                pass  # best-effort cleanup — doesn't affect the result either way

        if params.get("save", True):
            out = _output_path(path, "summary", ".txt")
            out.write_text(result, encoding="utf-8")
            summary_part = result.split("SUMMARY:", 1)[-1].strip() if "SUMMARY:" in result else result
            return f"Transcribed and summarized. Saved: {out.name}\n\nSummary: {summary_part[:500]}"
        return result
    ...
    return f"Unknown video action: '{action}'. Try: info, trim, extract_audio, extract_frame, compress, transcribe, summarize, convert"
