"""
dashboard/server.py — LITE Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "LITE Uploads",
        Path.home() / "Documents" / "LITE Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"LITE Dashboard Port {port}"
        prog_rule  = "LITE Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="lite_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until LITE is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "lite.key").exists() and (certs / "lite.crt").exists()
    ...
    def _build_app(self) -> "FastAPI":
        ...
        # Explicit no-cache headers on the HTML entry points — without this,
        # some mobile browsers cache these pages aggressively (especially
        # over HTTPS with a self-signed cert), which can leave a phone
        # showing a stale version of the app after an update.
        _NO_CACHE = {"Cache-Control": "no-store, no-cache, must-revalidate", "Pragma": "no-cache"}

        @app.get("/login", response_class=HTMLResponse)
        async def login_page():
            return HTMLResponse(self._login_html, headers=_NO_CACHE)
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in LITE to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('lite_token','{tok}');
  sessionStorage.setItem('lite_key','{key}');
  localStorage.setItem('lite_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to LITE…</p>
</body></html>""")
